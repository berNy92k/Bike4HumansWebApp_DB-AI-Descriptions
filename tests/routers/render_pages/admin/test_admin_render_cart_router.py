...
def test_render_carts_page_authenticated(client, seeded_data):
    # Given
    _login(client)

    # When
    response = client.get("/admin/carts/list")

    # Then
    assert response.status_code == 200


def test_render_carts_page_unauthenticated_redirects_to_login(client, seeded_data):
    # Given

    # When
    response = client.get("/admin/carts/list", follow_redirects=False)
