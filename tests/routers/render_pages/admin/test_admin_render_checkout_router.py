...
def test_render_checkouts_page_authenticated(client, seeded_data):
    # Given
    _login(client)

    # When
    response = client.get("/admin/checkouts/list")

    # Then
    assert response.status_code == 200


def test_render_checkouts_page_unauthenticated_redirects_to_login(client, seeded_data):
    # Given

    # When
    response = client.get("/admin/checkouts/list", follow_redirects=False)
